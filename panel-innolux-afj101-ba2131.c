...
static inline struct afj101_ba2131 *panel_to_afj101(struct drm_panel *panel)
{
	return container_of(panel, struct afj101_ba2131, panel);
}

/*
 * Init sequence based on register dump of a working panel
 */
static const struct panel_init_cmd afj101_ba2131_init_cmds[] = {
	...
	{ .dtype = 0x15, .wait = 0x78, .dlen = 0x02, .data = (char[]){ 0x11, 0x00 }},
...
static int afj101_ba2131_prepare(struct drm_panel *panel)
{
	...
	for (i = 0; i < ARRAY_SIZE(afj101_ba2131_init_cmds); i++) {
		...
		case MIPI_DSI_GENERIC_LONG_WRITE:
			ret = mipi_dsi_generic_write(dsi, cmd->data, cmd->dlen);
			break;
		case MIPI_DSI_DCS_SHORT_WRITE:
		case MIPI_DSI_DCS_SHORT_WRITE_PARAM:
		case MIPI_DSI_DCS_LONG_WRITE:
			ret = mipi_dsi_dcs_write_buffer(dsi, cmd->data, cmd->dlen);
			break;
		default:
			return -EINVAL;
		}
